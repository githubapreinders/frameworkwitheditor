...
.filter('duplicateFilter', function() {
...
.controller('UploadConfigurationsCtrl', ['$scope', 'Api', function($scope, Api) {
	...
	$scope.handleFile = function(files) {
		...
		var name = files[0].name.replace(/[/\s]+/, '');
		...
		if($scope.form.version === '')
		{
			$scope.form.version = Date.now();
		}
		
		$scope.name = name;
		$scope.file = files[0]; 
		console.log($scope.file, $scope.version, $scope.name);
	};

	$scope.submit = function() {
		if($scope.file == null){console.log("file is null..."); return;}
